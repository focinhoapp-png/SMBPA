...
export default function PetGrid() {
  const filteredPets = [...mockPets].reverse().filter((pet) => pet.status !== 'adotado').slice(0, 4);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredPets.map((pet, index) => (
            ...
            >
              <Link to={`/descricao-pet/${pet.id}`} className="h-52 w-full overflow-hidden relative block">
                <img
                  src={pet.image}
                  alt={pet.name}
              ...
              <div className="p-5 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-bold text-guapi-green">{pet.name}</h3>
                </div>
